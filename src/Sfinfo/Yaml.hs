...
transformYaml callBack inputDocument =
  ...
  where
    -- Extract the HsYaml events of bytestring
    inputEvents = map eEvent . rights . parseEvents
    -- convert an object to HsYaml event stream
    objToEvent obj = objBody
      where
        doc = inputEvents $ BSL.fromStrict $ Data.Yaml.encode obj
        (_, objBody, _) = headBodyTail doc 2
    -- encode a mapping stream to a proper yaml document
    encodeSubMapBS yMap =
      BSL.toStrict $
      writeEvents UTF8 $
      [StreamStart, DocumentStart NoDirEndMarker] <> yMap <> [DocumentEnd False, StreamEnd]
    processMap :: [(Maybe Text, Event)] -> [Event]
    processMap tupleMap =
      -- trace ("\n==DEBUG== " <> T.unpack (map (show . snd) innerMapEncoded) <> "\n==END==\n")
      outputMap
      where
